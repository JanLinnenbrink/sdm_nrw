...
train_dat <- readRDS("mnt/e:/sdm_nrw/train_dat")
...
occs <- st_read("mnt/c/0_Msc_Loek/M7_Fernerkundung/sdm_nrw/R/occurences.gpkg") %>%
  mutate(FID = 1:nrow(.)) %>% 
  .[,"FID"]
...
st_write(points, "~/R_output/training_points.shp", append=FALSE)
...
ggsave("~/R_output/points.pdf", pnts_plot)
...
saveRDS(trainDat, "~/R_output/sdm_nrw/trainDat")
...
vimp <- ggplot(varImp(model_ffs)) +
  geom_bar(width=1, stat="identity")

ggsave("~/R_output/vimp.pdf", vimp)

saveRDS(model_ffs,file="~/R_output/sdm_nrw/ffs_model") 
