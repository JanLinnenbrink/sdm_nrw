cv_estimation <- function(pts=pts_clustered, predictors=predictors_global) {
  ...
  ds <- lapply(c(1:length(possible_n)), function(x) {
    dist <- cv_dist(x)
    if (dist[[1]]$p.value > 0.05) {
      stop("samples are randomly distributed")
    } else {
      distances <- dist[[2]]$statistic[[1]]
      return(distances)
      }
  }
  )
  ...
  # get the first non-zero element of the list (= chosen block size)
  d_df <- data.frame(distances=distances, n = possible_n)
  n_sel <- d_df[min(d_df$distances),]$n
