...
vc2ras <- function(path, resolution, fields) {
  ...
  ras_ls <- list()
  for(i in seq_along(env_vector)) {ras_ls[[i]]=terra::rasterize(env_spatvec[[i]],raster_template,field=fields[[i]]);return(x)}
  
  env_raster <- stack(ras_ls)
}

env_data <- vc2ras(path=path, resolution = 10, fields = fields)
